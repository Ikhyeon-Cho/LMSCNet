...
def load(model, optimizer, scheduler, resume, path):
    '''
    ...
    # If resume, check that path exists and load everything to return
        ...
        assert os.path.isfile(
            file_path), '=> No checkpoint found at {}'.format(path)
...
def save(path, model, optimizer, scheduler, epoch, config):
    ...
    weights_fpath = os.path.join(
        path, 'weights_epoch_{}.pth'.format(str(epoch).zfill(3)))

    torch.save({
        # To start on next epoch when loading the dict...
        'startEpoch': epoch+1,
