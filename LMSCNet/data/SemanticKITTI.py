...
class SemanticKITTIDataset(Dataset):
    ...
    def __init__(self, cfg_dataset, phase):
        '''
        ...
        self.kitti_yaml = yaml.safe_load(
            open(os.path.join(yaml_path, 'semantic-kitti.yaml'), 'r'))

        # load from semantic-kitti.yaml
        self.nbr_classes = self.kitti_yaml['nbr_classes']
        self.grid_dimensions = self.kitti_yaml['grid_dims']   # [W, H, D]
        self.remap_lut = self.get_remap_lut(self.kitti_yaml)

        # manual values
        # images mean:  [88.61137282 93.70029365 92.11530949]
        self.rgb_mean = np.array([0.34749558, 0.36745213, 0.36123651])
        # images std:  [78.02753826 79.80311686 81.43122464]
        self.rgb_std = np.array([0.30599035, 0.3129534, 0.31933814])

        # load from config file
        self.root_dir = cfg_dataset['ROOT_DIR']
        self.modalities = cfg_dataset['MODALITIES']
        self.data_augmentation = {
            'FLIPS': cfg_dataset['AUGMENTATION']['FLIPS']}

        # semantic-kitti properties
        ...
        for modality in self.modalities:
            if self.modalities[modality] is True:
    ...
    def __len__(self):
        # TODO: Pass to something generic
        return len(self.filepaths['3D_OCCUPANCY'])

    def __getitem__(self, idx):
        # dataloader pushes data modality set at once
        data = {}
        do_flip = 0
        if self.phase == 'train' and self.data_augmentation['FLIPS']:
            do_flip = random.randint(0, 3)

        for modality in self.modalities:
            if self.modalities[modality] is True and modality in self.filepaths:
                data[modality] = self.get_data(modality, idx, do_flip)
    ...
    def get_filepaths(self, modality):
        ...
        sequences = list(sorted(glob(os.path.join(
            self.root_dir, 'dataset', 'sequences', '*')))[i] for i in self.split[self.phase])
        ...
        if self.phase != 'test':
            ...
            if modality == '3D_LABEL':
                ...
                for sequence in sequences:
                    assert len(os.listdir(sequence)) > 0, 'Error, No files in sequence: {}'.format(
                        sequence)
                    # Scale 1:1
                    self.filepaths['3D_LABEL']['1_1'] += sorted(
                        glob(os.path.join(sequence, 'voxels', '*.label')))
                    self.filepaths['3D_INVALID']['1_1'] += sorted(
                        glob(os.path.join(sequence, 'voxels', '*.invalid')))
                    # Scale 1:2
                    self.filepaths['3D_LABEL']['1_2'] += sorted(
                        glob(os.path.join(sequence, 'voxels', '*.label_1_2')))
                    self.filepaths['3D_INVALID']['1_2'] += sorted(
                        glob(os.path.join(sequence, 'voxels', '*.invalid_1_2')))
                    # Scale 1:4
                    self.filepaths['3D_LABEL']['1_4'] += sorted(
                        glob(os.path.join(sequence, 'voxels', '*.label_1_4')))
                    self.filepaths['3D_INVALID']['1_4'] += sorted(
                        glob(os.path.join(sequence, 'voxels', '*.invalid_1_4')))
                    # Scale 1:8
                    self.filepaths['3D_LABEL']['1_8'] += sorted(
                        glob(os.path.join(sequence, 'voxels', '*.label_1_8')))
            ...
            if modality == '3D_OCCLUDED':
                ...
                for sequence in sequences:
                    assert len(os.listdir(sequence)) > 0, 'Error, No files in sequence: {}'.format(
                        sequence)
                    self.filepaths['3D_OCCLUDED'] += sorted(
                        glob(os.path.join(sequence, 'voxels', '*.occluded')))
        ...
        if modality == '3D_OCCUPANCY':
            ...
            for sequence in sequences:
                assert len(os.listdir(sequence)) > 0, 'Error, No files in sequence: {}'.format(
                    sequence)
                self.filepaths['3D_OCCUPANCY'] += sorted(
                    glob(os.path.join(sequence, 'voxels', '*.bin')))
    ...
    def get_data(self, modality, idx, flip):
        ...
        if modality == '3D_OCCUPANCY':
            ...
            OCCUPANCY = SemanticKittiIO.data_augmentation_3Dflips(
                flip, OCCUPANCY)
            return OCCUPANCY[None, :, :, :]

        elif modality == '3D_LABEL':
            LABEL_1_1 = SemanticKittiIO.data_augmentation_3Dflips(
                flip, self.get_label_at_scale('1_1', idx))
            LABEL_1_2 = SemanticKittiIO.data_augmentation_3Dflips(
                flip, self.get_label_at_scale('1_2', idx))
            LABEL_1_4 = SemanticKittiIO.data_augmentation_3Dflips(
                flip, self.get_label_at_scale('1_4', idx))
            LABEL_1_8 = SemanticKittiIO.data_augmentation_3Dflips(
                flip, self.get_label_at_scale('1_8', idx))
            return {'1_1': LABEL_1_1, '1_2': LABEL_1_2, '1_4': LABEL_1_4, '1_8': LABEL_1_8}

        elif modality == '3D_OCCLUDED':
            OCCLUDED = SemanticKittiIO._read_occluded_SemKITTI(
                self.filepaths[modality][idx])
            OCCLUDED = np.moveaxis(OCCLUDED.reshape([self.grid_dimensions[0],
                                                     self.grid_dimensions[2],
                                                     self.grid_dimensions[1]]), [0, 1, 2], [0, 2, 1])
            OCCLUDED = SemanticKittiIO.data_augmentation_3Dflips(
                flip, OCCLUDED)
            return OCCLUDED
    ...
    def get_label_at_scale(self, scale, idx):
        ...
        INVALID = SemanticKittiIO._read_invalid_SemKITTI(
            self.filepaths['3D_INVALID'][scale][idx])
        LABEL = SemanticKittiIO._read_label_SemKITTI(
            self.filepaths['3D_LABEL'][scale][idx])
        if scale == '1_1':
    ...
    def get_inv_remap_lut(self):
        ...
        maxkey = max(self.kitti_yaml['learning_map_inv'].keys())

        # +100 hack making lut bigger just in case there are unknown labels
        remap_lut = np.zeros((maxkey + 1), dtype=np.int32)
        remap_lut[list(self.kitti_yaml['learning_map_inv'].keys())] = list(
            self.kitti_yaml['learning_map_inv'].values())

        return remap_lut

    def get_remap_lut(self, cfg_kitti):
        '''
        remap_lut to remap classes of semantic kitti for training...
        :return:
        '''
        ...
        # make lookup table for mapping
        maxkey = max(cfg_kitti['learning_map'].keys())

        # +100 hack making lut bigger just in case there are unknown labels
        remap_lut = np.zeros((maxkey + 100), dtype=np.int32)
        remap_lut[list(cfg_kitti['learning_map'].keys())] = list(
            cfg_kitti['learning_map'].values())
